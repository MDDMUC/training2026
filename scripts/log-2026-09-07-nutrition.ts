// Log Martin's 2026-09-07 food + Soul watch burn (manual estimate — no Anthropic).
// Safe to re-run: upserts by description match; sets session activity_calories = 500.
...
// Assumptions (stated in session log / chat):
// - Eggs: 2 large, plain (same as 2026-09-04 beef meal)
// - Serrano: FEN 100 g = 241 kcal / 31 P / 0 C / 13 F
// - WW buns: 8 × ~50 g "smaller than a fist" ≈ 135 kcal / 5 P / 24 C / 2 F each
// - Tuna: EU-style cans ~80 g drained solids each (al natural) ≈ 90 / 20 / 0 / 1
const entries: Entry[] = [
  {
    key: '%egg%',
    description: '2 boiled eggs, plain',
    calories: 145,
    protein_g: 13,
    carbs_g: 1,
    fat_g: 10,
  ...
  {
    key: '%bun%',
    description: '8 whole wheat buns (smaller than a fist)',
    calories: 1080,
    protein_g: 40,
    carbs_g: 192,
    fat_g: 16,
    items: [
      {
        ...
        qty: '8 × ~50 g (smaller than a fist)',
        calories: 1080,
        protein_g: 40,
        carbs_g: 192,
        fat_g: 16
  ...
  {
    key: '%tuna%',
    description: '2 cans tuna in water',
    calories: 180,
    protein_g: 40,
    carbs_g: 0,
    fat_g: 2,
    items: [
      {
        food: 'Tuna in water',
        qty: '2 cans · ~80 g drained each (EU al natural)',
        calories: 180,
        protein_g: 40,
        carbs_g: 0,
        fat_g: 2
...
try {
  ...
  const totals = sumEntries(dayEntries.map((e) => ({
